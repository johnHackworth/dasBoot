...
(function() {
  ...
  shipMarker.prototype = {
    views: {
      "EarlyBattleship": "assets/vehicles/earlBattleship_1_side.png",
      "EarlyDestroyer": "assets/vehicles/earlyDestroyer_1_side.png",
    },
    markerWidth: 10,
    markerHeight: 5,
    init: function() {
    ...
    addData: function() {
      ...
      this.name.x = this.marker.x - this.name.width / 2;
      this.name.y = this.marker.y + 20;
      this.name.visible = false;
      this.country = this.stage.addText(this.ship.country.name, {
        fontSize: '8px',
        color: '#333333'
      }, this.destroyables);

      this.country.x = this.marker.x - this.country.width / 2;
