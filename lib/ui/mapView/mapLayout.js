...
(function() {
  ...
  mapLayout.prototype = {
    ...
    addPathLine: function() {
      ...
      var tile = null;
      for (var i = 0, l = player.path.length; i < l; i++) {
        var sector = player.path[i];
        var nSector = player.path[i + 1];
        tile = this.tiles[sector.x][sector.y];
        var dots = this.getDotsPosition(player.path, i);
        // this.pathLine.drawCircle(tile.x + tile.tileWidth / 2, tile.y + 2 + tile.tileHeight / 2, 4);
        if (dots[0]) {
          this.pathLine.drawCircle(dots[0][0], dots[0][1], 4);
        }
        if (dots[1]) {
          this.pathLine.drawCircle(dots[1][0], dots[1][1], 4);
        } else {
          this.pathLine.drawCircle(tile.x + tile.tileWidth / 2, tile.y + 2 + tile.tileHeight / 2, 4);
        }
        if (dots[2]) {
          this.pathLine.drawCircle(dots[2][0], dots[2][1], 4);
        }
      }
