...
(function() {
  ...
  submarineView.prototype = {
    ...
    init: function() {
      ...
      this.zoomableContainer.addChild(this.peopleContainer);
      this.zoomableContainer.addChild(this.lightContainer);
    ...
    setSeaDeep: function() {
      ...
      this.seaView.y = y;
      window.svo = this.seaViewOscillation;
      if (this.counter % 6 === 0) {
        this.seaViewOscillation.originY = this.initSea + y - Math.floor(svo.particleHeight / 2);
        if (y <= -180) {
    ...
    adjustScale: function(speed) {
      ...
      if (this.targetScale > this.currentScale) {
        console.log(this.targetScale, this.currentScale)
