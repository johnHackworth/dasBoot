...
(function() {
  ...
  submarineView.prototype = {
    ...
    initMask: function() {
      ...
      myMask.lineTo(window.boot.config.width, 650);
      myMask.lineTo(0, 650);
      myMask.lineTo(0, 280);
      myMask.endFill();
      this.mask = myMask;
    ...
    addSea: function() {
      ...
      this.skyView = this.stage.addBackgroundToContainer(this.container, 0, this.initSky, window.boot.config.width, 250, this.sector.weather.skyColor, 1);
      this.skyView.tint = this.sector.weather.tint;
      this.seaView = this.stage.addBackgroundToContainer(this.container, 0, this.initSea, window.boot.config.width, 600, 0x0066AA, 1);
    ...
    addPlayerBoat: function() {
      ...
      this.container.addChild(this.submarineBackground);
    ...
    seaWaves: function() {
      var deep = this.stage.playerBoat.deep;
      if (deep < 40) {
        var randX = Math.randInt(this.seaView.width);
        var bubbles = new pixEngine.ParticleGenerator({
          stage: this.stage,
          before: this.seaView,
          bottom: this.initSea + this.seaView - 10,
          origin: {
            x: randX,
            y: this.initSea + this.seaView.y / 4
          },
          randomOrigin: {
            x: 200,
            y: 0
