...
window.boot.extensions.personMorale.prototype = {
  ...
  checkMoraleStance: function(worker2) {
    ...
    if (worker2.model.attributes.perks.indexOf('attractive') >= 0 &&
      ...
      this.model.attributes.kinsley > Math.randInt()
    ) {
      inspiration += 0.5;
      morale += 0.05;
    ...
    if (worker2.model.attributes.perks.indexOf('rude') >= 0) {
      morale -= 0.02;
      this.moraleModiffiers.push('negative');
    }
    if (this.model.attributes.perks.indexOf('racist') >= 0 &&
      worker2.model.culture != this.model.culture
    ) {
      morale -= 0.1;
      this.moraleModiffiers.push('racist');
    }
    if (this.model.attributes.perks.indexOf('homophobic') >= 0 &&
      worker2.model.attributes.kinsley > 70
    ) {
      inspiration -= 0.5;
      morale -= 0.05;
